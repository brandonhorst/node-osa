...
module.exports = function osa(osaFunction) {
  ...
  exec(executeString, function (err, stdout, stderr) {
    ...
    //If anything other than an `osascript` message was written to stderr,
    // it was a console.log statement. Save it;
    var reg = new RegExp('^' + consoleLogPrefix + ' ([\\s\\S]*?) ' + consoleLogSuffix + '$', 'gm');
    var matches = [];
    var log;
    var found;
    while (found = reg.exec(stderr)) {
      matches.push(found[1]);
      reg.lastIndex -= found[0].split(':')[1].length;
    }

    log = matches.length > 0 ? matches.join('\n') : null;
